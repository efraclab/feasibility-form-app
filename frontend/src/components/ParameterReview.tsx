...
export default function ParameterReview({
  ...
  const [adminBatchView, setAdminBatchView] =
  ...
  const isAdmin = () => {
    return (
      ...
      employeeId?.trim().toLowerCase() === "admin"
    );
  };
    ...
    try {
      ...
      allParams.forEach((param) => {
        if (param.batchId) {
          ...
          // Uploaded/completed batches remain visible to Admin
          // so they can be reopened and reverted if needed.
          if (
            status === "Uploaded" &&
            !isAdmin()
          ) {
          ...
          else if (status === "Uploaded") batch.uploadedCount++;
        }
      });
  ...
  const filteredBatches = batches.filter((batch) => {
    ...
    const isCompletedBatch =
      batch.totalCount > 0 &&
      batch.uploadedCount === batch.totalCount;

    const matchesAdminView =
      !isAdmin() ||
      (adminBatchView === "completed"
        ? isCompletedBatch
        : !isCompletedBatch);
    ...
    return matchesAdminView && matchesSearch;
  });
  ...
  const totalUploaded = batches.reduce((sum, b) => sum + b.uploadedCount, 0);
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6">
        ...
        {isAdmin() && (
          <div className="mb-6 flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-2 shadow-sm">
            <button
              type="button"
              onClick={() => setAdminBatchView("active")}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                adminBatchView === "active"
            ...
            <button
              type="button"
              onClick={() => setAdminBatchView("completed")}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                adminBatchView === "completed"
                      ...
                      <div className={`grid ${
                        ...
                        {isAdmin() && (
                          <div className="text-center border-l border-slate-300">
                            ...
                            <p className="text-[9px] text-indigo-600 uppercase tracking-wide">
                              Uploaded
                            </p>
                          </div>
                        )}
