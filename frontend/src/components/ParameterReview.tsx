...
export default function ParameterReview({
  ...
  const isReviewer = () => {
    return (
      role?.trim().toUpperCase() === "REVIEWER" ||
      ["EC57285", "EC56887"].includes(employeeId)
    );
  };
    ...
    try {
      ...
      allParams.forEach((param) => {
        if (param.batchId) {
          ...
          // Admin final upload marks buffer rows as Uploaded.
          // Keep completed uploaded batches visible to Reviewer so the
          // populated Reviewer Template can be opened/downloaded.
          if (status === "Uploaded" && !isReviewer()) return;
